...
/**
 * 管理员菜单 + system:admin:list|create|update|delete 权限（幂等）。
 * 落地方式：复制本文件到 database/seeds/ 后 `php think seed:run -s AdminMenuSeeder`，
...
class AdminMenuSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if ($dirId === 0) {
            echo "[AdminMenuSeeder] 未找到系统管理目录(System)，请先跑 AuthSeeder。\n";
            return;
        }

        $menuId = (int) Db::name('menu')->where(['tenant_id' => 0, 'name' => 'SystemAdmin'])->whereNull('deleted_at')->value('id');
        if ($menuId === 0) {
            $menuId = (int) Db::name('menu')->insertGetId([
                'tenant_id' => 0, 'parent_id' => $dirId, 'type' => 2, 'name' => 'SystemAdmin', 'title' => '管理员',
                'path' => '/system/admin', 'component' => 'system/admin/index', 'perms' => '', 'icon' => 'menu',
