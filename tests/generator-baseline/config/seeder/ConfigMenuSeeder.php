...
/**
 * 配置中心菜单 + system:config:list|create|update|delete 权限（幂等）。
 * 落地方式：复制本文件到 database/seeds/ 后 `php think seed:run -s ConfigMenuSeeder`，
    ...
    public function run(): void
    {
        ...
        if ($menuId === 0) {
            $menuId = (int) Db::name('menu')->insertGetId([
                'tenant_id' => 0, 'parent_id' => $dirId, 'type' => 2, 'name' => 'SystemConfig', 'title' => '配置中心',
                'path' => '/system/config', 'component' => 'system/config/index', 'perms' => '', 'icon' => 'menu',
