...
/**
 * 字典类型菜单 + system:dict:list|create|update|delete 权限（幂等）。
 * 落地方式：复制本文件到 database/seeds/ 后 `php think seed:run -s DictMenuSeeder`，
 * 或并入既有种子流程；依赖 AuthSeeder 已建「系统管理」目录(System)。
 */
class DictMenuSeeder extends Seeder
{
    public function run(): void
    {
        ...
        if ($dirId === 0) {
            echo "[DictMenuSeeder] 未找到系统管理目录(System)，请先跑 AuthSeeder。\n";
            return;
        }
        ...
        if ($menuId === 0) {
            $menuId = (int) Db::name('menu')->insertGetId([
                'tenant_id' => 0, 'parent_id' => $dirId, 'type' => 2, 'name' => 'SystemDict', 'title' => '字典类型',
                'path' => '/system/dict', 'component' => 'system/dict/index', 'perms' => '', 'icon' => 'menu',
