...
// +----------------------------------------------------------------------
// | @project   BenXinAdmin
// | @mission   服务 — 角色 CRUD + 分配菜单（同步 Casbin，生成器复刻 role 授权链路母版）
// | @author    仗键天涯(daxing)
...
/**
 * 角色服务：CRUD + 覆盖式分配菜单并同步 Casbin（生成器复刻 role 授权链路母版）。
 * super_admin 为内置受保护行：不可删/停/改 code/分配菜单。
 * code 唯一含软删（§5.1）。
 */
class RoleService extends BxService
{
    ...
    /**
     * 分页列表（keyword: name/code；status 精确）。
     *
    ...
    public function update(int $id, array $data): Role
    {
        ...
        if ($role->code === 'super_admin') {
            if (array_key_exists('code', $data) && $data['code'] !== 'super_admin') {
                throw new BusinessException('超级管理员为内置保护数据，不可修改 code');
            }
            if (array_key_exists('status', $data) && (int) $data['status'] !== 1) {
                throw new BusinessException('超级管理员为内置保护数据，不可停用');
            }
        }
     ...
     * 否则事务软删 + 清 role_menu/role_dept + 清该角色 casbin 策略 + reload。
     */
    public function delete(int $id): void
    {
        $role = Role::findOrFail($id);
        if ($role->code === 'super_admin') {
            throw new BusinessException('超级管理员为内置保护数据，不可删除');
        }

        if (Db::name('admin_role')->where('role_id', $id)->count() > 0) {
            throw new BusinessException('该角色已被管理员绑定，无法删除');
        }
    ...
    {
        $role = Role::findOrFail($id);
        if ($role->code === 'super_admin' && $status !== 1) {
            throw new BusinessException('超级管理员为内置保护数据，不可停用');
        }
        $role->status = $status;
        $role->save();
    ...
    public function assignMenus(int $id, array $menuIds): void
    {
        $role = Role::findOrFail($id);
        if ($role->code === 'super_admin') {
            throw new BusinessException('超级管理员为内置保护数据，不可分配菜单');
        }
