...
/**
 * 素材菜单 + system:resource:list|create|update|delete 权限（幂等）。
 * 落地方式：复制本文件到 database/seeds/ 后 `php think seed:run -s ResourceMenuSeeder`，
...
class ResourceMenuSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $menuId = (int) Db::name('menu')->where(['tenant_id' => 0, 'name' => 'ResourceList'])->whereNull('deleted_at')->value('id');
        if ($menuId === 0) {
            $menuId = (int) Db::name('menu')->insertGetId([
                'tenant_id' => 0, 'parent_id' => $dirId, 'type' => 2, 'name' => 'ResourceList', 'title' => '素材',
                'path' => '/resource/list', 'component' => 'resource/list/index', 'perms' => '', 'icon' => 'picture',
