...
/**
 * 岗位服务：标准 CRUD（复刻 M1-C 角色样板）。
 * code 唯一含软删（§5.1）；删除时若有管理员绑定该岗位则拒绝。
 */
class PostService extends BxService
{
    ...
    /**
     * 删除：有管理员绑定该岗位（bx_admin_post）拒绝；否则软删。
     */
    public function delete(int $id): void
    {
        $post  = Post::findOrFail($id);
        $bound = Db::name('admin_post')->where('post_id', $id)->count();
        if ($bound > 0) {
            throw new BusinessException("仍有 {$bound} 个管理员绑定该岗位，请先解绑");
        }
